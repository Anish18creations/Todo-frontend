...
export default function Settings() {
  ...
  const handlesubmit = async (e) => {
    ...
    if (old) {
      if (old !== ogpassword) {
        toast.error("Please provide your original password correctly to update password!!", { duration: 2000 });
        return;
      }
    }
    ...
    if (old && !formdata.password) {
      toast.error("Please remove your original password if you don't want to update password!!", { duration: 2000 });
    ...
    if (response == undefined) {
      toast.error('Please provide valid name and/or password to update!!', { duration: 1000 });
      return;
    }
    ...
    if (response.success == 'p') {
      ...
      newState.password = '';
      newState.name = localStorage.getItem('username');
